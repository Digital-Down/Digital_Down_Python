...
class BetaPhaseExtraction:
    ...
    @classmethod
    def convert(cls, audio_data, oscillations, threshold=100, max_threshold=32767, channel=0):
        state = 'positive'
        beta_negative_positive = []
        beta_negative_negative = []
        current_oscillation = []
        ...
        for i, sample in enumerate(audio_data):
            ...
            if state == 'positive':
                if 1 <= value <= 16384:
                    beta_negative_positive.append(value)
                    current_oscillation.append(value)
                elif value < 0:
                    state = 'negative'
                    beta_negative_negative.append(value)  # Keep negative value
                    current_oscillation.append(value)
                    zero_crossings += 1
                ...
                if -16384 <= value <= -1:
                    ...
                    cls.process_oscillation(beta_negative_positive, beta_negative_negative, current_oscillation, oscillations, threshold)
                    state = 'positive'
                    beta_negative_positive = [value] if 1 <= value <= 16384 else []
                    beta_negative_negative = []
                    current_oscillation = [value] if 1 <= value <= 16384 else []
                    zero_crossings += 1
    ...
    @classmethod
    def process_oscillation(cls, beta_negative_positive, beta_negative_negative, current_oscillation, oscillations, threshold):
        total_count = len(beta_negative_positive) + len(beta_negative_negative)
        beta_negative_positive_count = len(beta_negative_positive)
        beta_negative_negative_count = len(beta_negative_negative)
        avg_abs_value = cls.average_absolute_value(current_oscillation)

        if threshold <= avg_abs_value <= 32767:
            found_match = False
            for note in cls.get_all_notes():
                ...
                if abs(total_count - expected_count) <= 1:  # Allow for rounding error
                    ...
                    print(f"Accepted oscillation for {note}, total count: {total_count}, beta- positive count: {beta_negative_positive_count}, beta- negative count: {beta_negative_negative_count}")
                    found_match = True
                    break
            if not found_match:
                print(f"Rejected oscillation, total count: {total_count}, beta- positive count: {beta_negative_positive_count}, beta- negative count: {beta_negative_negative_count}")
        elif avg_abs_value > 32767:
            print(f"Rejected oscillation due to high average absolute value, total count: {total_count}, beta- positive count: {beta_negative_positive_count}, beta- negative count: {beta_negative_negative_count}")
        else:
            print(f"Rejected oscillation due to low average absolute value, total count: {total_count}, beta- positive count: {beta_negative_positive_count}, beta- negative count: {beta_negative_negative_count}")
    ...
    @classmethod
    def save_oscillations(cls, note, samples, output_folder):
        if len(samples) > 0:
    ...
    def process_audio_folder(cls, input_folder, output_folder, incomplete_folder=None, threshold=100, max_threshold=32767, methylphenethylamine=False):
        ...
        def process_audio_file(file_path, file_type):
            ...
            for note, samples in oscillations.items():
                cls.save_oscillations(note, samples, output_folder)
        ...
        def process_file(item_path):
            ...
            if methylphenethylamine:
                ...
                AudioSampleValues.list_to_wav(
                    Methylphenethylamine.slow([AudioSampleValues.wav_to_list(processed_path)], 1),
                    slow_output
                )
                AudioSampleValues.list_to_wav(
                    Methylphenethylamine.fast([AudioSampleValues.wav_to_list(processed_path)], 1),
                    fast_output
                )
    ...
    def process_audio_folder_normalize(cls, input_folder, output_folder, incomplete_folder=None, threshold=100, max_threshold=32767, methylphenethylamine=False):
        ...
        def process_file(item_path):
            ...
            if info.subtype != 'PCM_16' or info.samplerate != 44100 or info.channels != 2:
                ...
                cls.normalize_audio(normalized_path, normalized_path)
            else:
                print(f"Normalizing")
            ...
            if methylphenethylamine:
                ...
                AudioSampleValues.list_to_wav(
                    Methylphenethylamine.slow([AudioSampleValues.wav_to_list(normalized_path)], 1),
                    slow_output
                )
                AudioSampleValues.list_to_wav(
                    Methylphenethylamine.fast([AudioSampleValues.wav_to_list(normalized_path)], 1),
                    fast_output
                )
